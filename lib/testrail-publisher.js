...
const publishResultsToTestRail = (
      ...
      testrail
        ...
        .then(suites => {
          const existingTestRailSuite = _.find(
            suites.body,
            suite => suite.name.trim() === mochaSuiteName.trim()
          );
          ...
          console.log(`Existing suite not found. Creating suite: '${suiteName}'`);
          return testrail
      ...
      testrail
        ...
        .then(sections => {
          const existingTestRailSection = _.find(
            sections.body,
            section => section.name.trim() === mochaSectionName.trim()
          );
          ...
          console.log(`No existing section found. Creating section: '${mochaSectionName}'`);
          return testrail
      ...
      testrail
        ...
        .then(cases => {
          const existingTestRailCase = _.find(
            cases.body,
            test => test.title.trim() === mochaTestTitle.trim()
          );
          ...
          console.log(`Existing test case not found. Creating test case: '${mochaTestTitle}'`);
          return testrail
  ...
  getSuiteId(testResults.suite.name)
    ...
    .then(sections => {
      ...
      if (milestoneId) {
        testRun.milestone_id = milestoneId;
      }
      return testrail.addRun(projectId, testRun);
    })
    .then(run => {
